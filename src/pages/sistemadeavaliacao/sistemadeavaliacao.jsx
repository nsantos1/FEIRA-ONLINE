...
/* ------------------ Componente Principal ------------------ */
 ...
 function SistemaDeAvaliacao() {
  const { id } = useParams();
  const produtoAtual = useMemo(() => produtos.find((p) => String(p.id) === String(id)), [id]);

  const { usuario, cadastrar, logout } = useUsuario();

  const inicializarAvaliacoes = useCallback((avs = []) => {
    return avs.map((a) => ({
      ...a,
      curtidasPor: Array.isArray(a.curtidasPor) ? a.curtidasPor : [],
      descurtidasPor: Array.isArray(a.descurtidasPor) ? a.descurtidasPor : [],
  ...
  const avaliacoesIniciais = useMemo(
    () => inicializarAvaliacoes(produtoAtual?.avaliacoes || []),
    [produtoAtual, inicializarAvaliacoes]
  );
      ...
      <main className="review-system-main-content">
        <div className="review-system-container">
          ...
          <p className="review-system-product-subtitle">
            Deixe sua opinião sobre nosso produto e veja o que outros clientes acharam.
          </p>
